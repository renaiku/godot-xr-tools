...
@export_flags_3d_physics var grab_collision_mask := DEFAULT_GRAB_MASK:
		set = _set_grab_collision_mask

@export_group("Ranged Grab")
...
@export_custom(PROPERTY_HINT_NONE, "suffix:m") var ranged_distance := 5.0:
		set = _set_ranged_distance

## Angle at which Pickables can be grabbed from afar
@export_range(0.0, 45.0, 1.0, "degrees") var ranged_angle := 5.0:
		set = _set_ranged_angle

## Physics layers that Pickables must be in to be picked up from afar
@export_flags_3d_physics var ranged_collision_mask := DEFAULT_RANGE_MASK:
...
## Add support for [method is_xr_class] on XRTools classes
